...
#     # Drop first to ensure clean schema with correct settings
#     local_client.command(f"DROP TABLE IF EXISTS {config.CLICKHOUSE_DATABASE}.{table}")

#     create_sql = f"""
#     CREATE TABLE {config.CLICKHOUSE_DATABASE}.{table} (
#         {', '.join(columns)}
#     ) ENGINE = MergeTree()
#     ORDER BY {order_by}
...
#             # For fct_coupons, limit to 50k rows for local testing (huge table)
#             if table == "fct_coupons":
#                 where_remote = (where_remote + " AND " if where_remote else "") + "coupon_id IN (SELECT coupon_id FROM main.fct_coupons ORDER BY coupon_id LIMIT 50000)"
...
"""
Sync tables from Remote ClickHouse to Local Docker ClickHouse.

Remote ClickHouse:
    Configure the values in the REMOTE_* section below.
...
# ============================================================
# REMOTE CLICKHOUSE CONFIG
# ============================================================
REMOTE_HOST = "clickhouse-test.waffarha.tech"
REMOTE_PORT = 443
REMOTE_USERNAME = "clickhouse_read"
REMOTE_PASSWORD = "clickhouse_p@ss2026"
REMOTE_DATABASE = "main"
REMOTE_SECURE = True


# ============================================================
# LOCAL DOCKER CLICKHOUSE CONFIG
# ============================================================
LOCAL_HOST = "localhost"
LOCAL_PORT = 8123
LOCAL_USERNAME = "default"
LOCAL_PASSWORD = "clickhouse123"
LOCAL_DATABASE = "main"
LOCAL_SECURE = False
...
def get_remote_client():
    print(
        f"Connecting to REMOTE ClickHouse: "
...
def get_local_client():
    print(
        f"Connecting to LOCAL ClickHouse: "
...
def create_local_table(local_client, remote_schema, table):
    columns = []
    ...
    # Choose a reasonable ORDER BY key
    if table == "dim_partners":
        order_by = "part_id"

    elif table == "dim_offers":
        order_by = "offer_id"

    elif table == "dim_type_price":
        order_by = "offer_id"

    elif table == "fct_coupons":
        order_by = "coupon_id"
        ...
        order_by = id_column if id_column else remote_schema[0][0]

    print(f"Creating local table: {LOCAL_DATABASE}.{table}")
    print(f"ORDER BY: {order_by}")

    local_client.command(
        f"""
        CREATE TABLE IF NOT EXISTS {LOCAL_DATABASE}.{table}
        (
            {columns_sql}
        )
        ENGINE = MergeTree
...
# SYNC ONE TABLE
# ============================================================
def sync_table(remote_client, local_client, table, batch_size=5000):
    print("\n" + "=" * 70)
    ...
    if not table_exists(
        ...
        print(
            f"ERROR: Remote table does not exist: "
            f"{REMOTE_DATABASE}.{table}"
        )
        return False

    print(
        f"Remote table found: "
        f"{REMOTE_DATABASE}.{table}"
    )
    ...
    # Get remote schema
    # --------------------------------------------------------
    remote_schema = get_table_schema(
        remote_client,
        REMOTE_DATABASE,
        table,
    )

    print(f"Columns found: {len(remote_schema)}")

    # --------------------------------------------------------
    # Create local table if missing
    # --------------------------------------------------------
    # --------------------------------------------------------
# Make sure local table has the same schema as remote
# --------------------------------------------------------
    if table_exists(
        local_client,
        LOCAL_DATABASE,
        ...
        print(
            f"Local table already exists: "
            f"{LOCAL_DATABASE}.{table}"
        )

        local_schema = get_table_schema(
            local_client,
            LOCAL_DATABASE,
            table,
        )
        ...
        local_columns = [
            (row[0], row[1])
            for row in local_schema
        ]

        if remote_columns != local_columns:
            print("\nSCHEMA MISMATCH!")
            print(
                f"Remote columns: {len(remote_columns)}"
            )
            print(
                f"Local columns : {len(local_columns)}"
            )

            print("\nRecreating local table...")
            ...
            print("Local table recreated successfully.")

        else:
            print("Schema matches remote.")

    else:
        print(
            f"Local table does not exist: "
            f"{LOCAL_DATABASE}.{table}"
        )
    ...
    # Remote count
    # --------------------------------------------------------
    remote_count_result = remote_client.query(
        f"""
        SELECT count()
        FROM {REMOTE_DATABASE}.{table}
        """
    )

    remote_count = remote_count_result.result_rows[0][0]

    print(f"Remote rows: {remote_count:,}")

    # --------------------------------------------------------
    # Local count before sync
    # --------------------------------------------------------
    local_count_result = local_client.query(
        f"""
        SELECT count()
    ...
    local_count_before = local_count_result.result_rows[0][0]

    print(
        f"Local rows before sync: "
        f"{local_count_before:,}"
    )

    # --------------------------------------------------------
    # IMPORTANT:
    # Only truncate if the table exists.
    # This is safe for dim_partners because it currently
    # doesn't exist locally.
    # --------------------------------------------------------
    print(
        f"Truncating local table "
        f"{LOCAL_DATABASE}.{table}..."
    )

    local_client.command(
        f"TRUNCATE TABLE {LOCAL_DATABASE}.{table}"
    )
    ...
    while offset < remote_count:
        ...
        print(
            f"Fetching rows "
            f"{offset:,} -> "
            f"{min(offset + batch_size, remote_count):,}"
        )

        result = remote_client.query(
            f"""
            SELECT {columns_sql}
            FROM {REMOTE_DATABASE}.{table}
            LIMIT {batch_size}
    ...
    local_count_after = local_count_result.result_rows[0][0]

    print("\nSYNC RESULT")
    print("-" * 50)
    print(f"Remote rows : {remote_count:,}")
    print(f"Inserted    : {total_inserted:,}")
    print(f"Local rows  : {local_count_after:,}")

    if local_count_after == remote_count:
...
def main():
    print("\n" + "=" * 70)
    print("WAFFARHA CLICKHOUSE SYNC")
    print("=" * 70)
    ...
    try:
        ...
        print(
            f"Remote test result: "
            f"{result.result_rows[0][0]}"
        )
